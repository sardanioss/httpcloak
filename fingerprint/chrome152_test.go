...
func TestChrome152Desktop(t *testing.T) {
	...
	for _, name := range []string{
		...
		// A GREASE signature algorithm leads signature_algorithms, and the
		// eleven behind it are Chrome 151's, unchanged.
		//
		// This is not a guess about a Finch rollout, it is what Chrome 152
		// shipped. From the 152.0.7977.64 release tag, net/base/features.cc:965:
		//
		//	BASE_FEATURE(kTlsGreaseSigalgs, base::FEATURE_ENABLED_BY_DEFAULT);
		//
		// and features.h calls it "a killswitch for behavior that is enabled by
		// default", so the flag exists to switch the behaviour OFF remotely, not
		// to roll it out gradually. ssl_client_socket_impl.cc:211 then calls
		...
		//
		// A previous release shipped this without the GREASE, on the theory that
		// a JA4 implementation which fails to strip GREASE from the sigalg list
		// would see one profile as many clients. It would, and does: twenty
		// connections to a non-compliant scorer gave twelve distinct JA4_c. But
		// that churn is what EVERY Chrome 152 produces, so it is the population,
		// not an anomaly, and the stable-JA4 profile was the odd one out.
		//
		// Check the release tag, never main, when the question is what a Chrome
		// version actually shipped:
		//   chromium.googlesource.com/chromium/src/+/refs/tags/<ver>/<path>?format=TEXT
...
// The killswitch state has to stay expressible.
//
// kTlsGreaseSigalgs is enabled by default in Chrome 152, so the shipping preset
// greases sigalgs. But it is a killswitch: Google can turn the behaviour off
// remotely without a Chrome release, and if that ever happens the profile has to
// be able to follow without one of ours either. A caller drops 2570 from the head
// of signature_algorithms in a JSON preset.
//
// Without this test, "switchable" is a claim in a comment rather than a property
// of the code, and an edit to the sigalg path could quietly remove the ability to
// express the other state at all.
func TestChrome152SigalgGreaseCanBeTurnedOff(t *testing.T) {
	const spec = `{
	  "preset": {
	    "name": "chrome-152-killswitched-sigalgs",
	    "based_on": "chrome-152-windows",
	    "tls": {
	      "signature_algorithms": [2308, 2309, 2310, 1027, 2052,
	...
	if err != nil {
		t.Fatalf("loading the killswitched variant failed: %v", err)
	}
	p, err := BuildPreset(pf.Preset)
	if err != nil {
		t.Fatalf("building the killswitched variant failed: %v", err)
	}

	if len(p.SignatureAlgorithms) != 11 {
		t.Fatalf("%d signature algorithms, want 11 (Chrome 151's list, no GREASE)",
			len(p.SignatureAlgorithms))
	}
	for i, sa := range p.SignatureAlgorithms {
		if uint16(sa)&0x0f0f == 0x0a0a {
			t.Errorf("algorithm %d is a GREASE value %#04x; the killswitched "+
				"variant carries none", i, sa)
		}
	}

	// And the default really is the other state, so the two are distinguishable.
	...
	if len(base.SignatureAlgorithms) == len(p.SignatureAlgorithms) {
		t.Errorf("the default and the killswitched variant both have %d signature "+
