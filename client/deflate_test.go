...
// TestDeflateMatchesStdlib holds every buffered deflate entry point to what the
// standard library's inflater produced for the same input. The streaming path
// is not compared: setupDecompressor has no deflate case and returns the body
// raw, which predates this change and is left as it is.
func TestDeflateMatchesStdlib(t *testing.T) {
	for i, input := range deflateCorpus(t) {
		...
		for name, fn := range map[string]func([]byte, string) ([]byte, error){
			"decompress": decompress, "decompressHTTP3": decompressHTTP3, "Decompress": Decompress,
		} {
			got, gotErr := fn(input, "deflate")
			sameInflate(t, name, i, got, gotErr, want, wantErr)
		}
	}
}
